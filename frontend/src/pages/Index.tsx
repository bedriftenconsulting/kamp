...
const Countdown = ({ targetDate }: { targetDate: string }) => {
  ...
  if (isStarted) return <div className="text-sm font-bold text-white bg-primary/40 px-3 py-1.5 rounded-sm inline-block mb-6 border border-primary-foreground/20">Tournament in progress!</div>;

  return (
    <div className="flex gap-2 sm:gap-4 mb-8">
      {Object.entries(timeLeft).map(([unit, value]) => (
        <div key={unit} className="flex flex-col items-center justify-center bg-black/50 backdrop-blur-md rounded-xl p-3 sm:p-4 min-w-[70px] sm:min-w-[90px] border border-white/10 shadow-xl">
          <span className="text-3xl sm:text-5xl font-black text-white">{String(value).padStart(2, '0')}</span>
          <span className="text-[10px] sm:text-xs uppercase tracking-[0.2em] text-white/70 font-bold mt-1.5">{unit}</span>
        </div>
      ))}
    </div>
  );
...
export default function Index() {
  ...
  const participantsByLevel = {
    Beginner: players.filter((p) => p.tennis_level === "Beginner" || p.tennis_level === "Beginner "),
    Intermediate: players.filter((p) => p.tennis_level === "Intermediate" || p.tennis_level === "Intermediate "),
    Advanced: players.filter((p) => p.tennis_level === "Advanced" || p.tennis_level === "Advanced "),
  };
        ...
        <div className="container relative py-20 md:py-32 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <div className="mb-6 flex items-center gap-3">
               <span className="px-3 py-1 font-bold text-[10px] tracking-widest uppercase bg-primary-foreground text-primary rounded-full shadow-lg">Next Tournament</span>
               <span className="px-3 py-1 font-bold text-[10px] tracking-widest uppercase bg-black/40 text-white rounded-full border border-white/20 backdrop-blur-md">
                 {tournament.status === "scheduled" ? "Upcoming" : tournament.status}
               </span>
            </div>
            
            {tournament.startDate && <Countdown targetDate={tournament.startDate} />}
            ...
            <div className="flex flex-wrap gap-4">
              <Link to="/live">
                <Button size="lg" className="gap-2 font-bold shadow-xl shadow-primary/20 bg-primary-foreground text-primary hover:bg-white transition-all hover:scale-105 active:scale-95 text-base">
              ...
              <Link to="/schedule">
                <Button size="lg" variant="outline" className="gap-2 font-bold bg-black/40 backdrop-blur-md border-white/20 text-white hover:bg-black/60 hover:text-white transition-all text-base shadow-xl">
                  <Calendar size={20} />
                  View Schedule
                </Button>
              </Link>
          ...
          >
             <div className="flex items-center justify-between mb-2">
               <h3 className="text-white font-black tracking-widest text-sm uppercase">Upcoming Events</h3>
             ...
             {upcomingTournaments.map((evt) => (
               ...
               >
                  <div className="h-16 w-16 rounded-lg overflow-hidden bg-primary/30 flex-shrink-0 shadow-inner">
                    <img src={evt.bannerImage || '/background.png'} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 ease-out" alt={evt.name} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="text-white font-bold text-lg leading-tight truncate">{evt.name}</h4>
                  ...
                  <div className="space-y-1">
                    <ol className="list-decimal list-inside text-sm">
                      {participantsByLevel[level].map((p: any) => (
                        <li key={p.id} className="text-muted-foreground py-0.5">
                          <span className="font-medium text-foreground">{p.name}</span>
                          {p.gender ? ` ${(p.gender || "").toLowerCase() === "men" || (p.gender || "").toLowerCase() === "male" ? "(M)" : "(W)"}` : ""}
                        </li>
                      ))}
                    </ol>
                  </div>
